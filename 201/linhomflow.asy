...
block ODE=roundrectangle("$a_2 y^{''}+ a_1 y^{'} + a_0 y =0$",(0,dy));

block chareqn=roundrectangle("$a_2 r^2 + a_1 r + a_0 =0$",(0,0));

block realneq=roundrectangle(pack(Label("$r_1\neq r_2$, both Real"),"",
				  "$y_1=e^{r_1t} ,y_2=e^{r_2 t}$"),(-dx,-dy));
block realeq=roundrectangle(pack(Label("$r_1=r_2$, both Real"),"",
...
block gsoln=roundrectangle("$y=C_1 y_1 + C_2 y_2$",(0,-2dy));
