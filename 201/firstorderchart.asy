...
block bernoulli=roundrectangle(pack(Label("Bernoulli DEs"),"",
				    "$\frac{dy}{dx} + Py = Qy^n$"), (-dx, dy));
block linear=roundrectangle(pack(Label("Linear DEs"),"",
				 "$\frac{dy}{dx} + Py = Q$"),(-dx,0));
...
block separable=roundrectangle(pack(Label("Separable DEs"),"",
				    "$h(y)\frac{dy}{dx} = g(x)$"),(0,-dy));
...
add(new void(picture pic, transform t) {
    ...
    linear--Down--Right
      --Label("$\mu = \exp \left(\int P(x) \,dx\right)$",align=S)
      --Arrow--separable;

    linear_coeffs--Label(minipage("{\centering% \
                                    $x=u+h,\; y=v+k$\\ \
                                    $a_1 h + b_1 k + c_1 =0$\\ \
                                    $a_2 h + b_2 k + c_2 =0$\\}"),align=E)
      --Arrow--hom;
    hom--Label("$z=\frac{y}{x}$",align=E)--Arrow--separable;
