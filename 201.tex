...
\eee
Ee need to invert the Laplace transform in order to determin $y$.
...
all DEs are linear. In fact, the most aren't. One 
...
\workedexample{
  ...
  \item The comparison test: if 
    $\lim_{n \rightarrow \infty} \frac{a_n}{b_n} \in (0,\infty)$, then $A$ 
    converges if and only if $B$ converges.
  \item The ratio test: if 
    $\lim_{n\rightarrow\infty}\abs{\frac{a_{n+1}}{a_n}}=c < 1$, then $A$ converges.
...
representations). Then differential equations of the form
\be
\label{singode}
y'' + p(t)y' + q(t)y=0
\ee
...
\eee
In order to satisfy equation \eqref{singode}, this coefficient must equal $0$. 
...
get the standard form of Bessel's function of the first kind of order $\a$,
  ...
  \(\frac{t}{2}\)^{2n-\a}.
  \eee
Where $\G(x)$ is called the Gamma function and it is just an extension of the 
...
are either $\sin$ or $\cos$, i.e.\ when $y$ is a Fourier sine or cosine series.
