...
\Laplace\[f\] = \int_0^\infty f(t) e^{-st} \, dt
$$
and takes a function of $t$ to a function of $s$. 
We use the notation $\Laplace\[f\] = F(s)$ in the following table:
...
\begin{tabular}{ l |  l }
  ...
  \hline
  $f'(t)$ & $sF(s) -f(0)$\\ \hline
  $f''(t)$ & $s^2F(s) -sf(0) - f'(0)$\\ \hline
  $f^{(n)}(t)$ & $s^nF(s) -s^{n-1}f(0)-\dots - f^{(n-1)}(0)$\\ \hline
  $t$ & $\frac{1}{s^2}$ \\ \hline
  $t^n$ & $\frac{n!}{s^{n+1}}$ \\ \hline
  $e^{\a t}$ & $\frac{1}{s-\a} $ \\ \hline
  $e^{\a t}f(t) $ & $F(s-\a) $ \\ \hline
  $f(ct) $ & $\frac{1}{c}F\(\frac{s}{c}\) $ \\ \hline
  $\sin(\beta t)$ & $\frac{\beta}{s^2 + \beta^2} $ \\ \hline
  $\cos(\beta t)$ & $\frac{s}{s^2 + \beta^2} $ \\ \hline
  $\sinh(\beta t)$ & $\frac{\beta}{s^2 - \beta^2} $ \\ \hline
  $\cosh(\beta t)$ & $\frac{s}{s^2 - \beta^2} $ \\ \hline
  $e^{\alpha t}\sin(\beta t)$ & $\frac{\beta}{(s-\alpha)^2 + \beta^2} $ \\ \hline
  $e^{\alpha t}\cos(\beta t)$ & $\frac{s-\alpha}{(s-\alpha)^2 + \beta^2} $ \\ 
  \hline
  $u_c(t)$ & $\frac{e^{-cs}}{s}$ \\ \hline
  $u_c(t)f(t-c)$ & $e^{-cs}F(s)$ \\ \hline
  $\delta(t)$ & $1$ \\ \hline
  $\int_0^t f(t-\tau) g(\tau) \,d\tau \doteq f*g$ & $F(s)G(s) $ \\ \hline
  $f(t)$ with $f(t+T)=f(t)$ & $\frac{\int_0^Tf(t) e^{-st}dt}{1-e^{-sT}}$\\ \hline
  $t^n f(t)$ & $(-1)^n \frac{d^n}{ds^n} F(s)$\\ \hline
\end{tabular}\\
$$
%\end{center}
