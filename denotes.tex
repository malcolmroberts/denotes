...
\cfoot{
  \begin{center}
    Copyright 2009-2015, Graduates at Alberta Mathematics Etc.
  \end{center}
  \doclicenseThis{}
}
...
by $f(x)$.
% TODO: from http://en.wikipedia.org/wiki/Self-adjoint_operator and
% the fact that the differential operators are self-adjoint, we get an
% orthongoal basis.
...
\workedexample{
  ...
  \end{dmath*}
  And in order to satisfy $u(x,0)=1+x+x^2$, we have
  \begin{dmath*}
  ...
  \begin{dmath*}
    ...
    = \frac{2}{n\pi}\[ -\pi^2\cos(n\pi)
    +\frac{2}{n}\(\left.x \sin(nx)\right|_0^\pi- \int_0^\pi\sin(nx)dx\)  \]
    = \frac{-2\pi}{n}\cos(n\pi)-\frac{4}{n^3\pi} \left[-\cos(nx) \right]_0^\pi
    = \frac{-2\pi}{n}(-1)^n + 4\frac{(-1)^n-1}{n^3\pi}
  \end{dmath*}
  The solution is therefore
  \begin{dmath*}
    u(x,t) = 1+x+
    \sum_{n=1}^\infty \[\frac{-2\pi}{n}(-1)^n + 4\frac{(-1)^n-1}{n^3\pi}\]
    e^{-\b n^2 t}\sin(nx).
  \end{dmath*}
% Useless figure
...
%%  LocalWords:  DEs ie eg IVP dy dx separableexample dF exactsol xy
%%  LocalWords:  exactimpl exactC dt linearf dv uv du linearexx IVPs
%%  LocalWords:  Thinsulate Joe's env Thinsulate's TODO Cx ax cy hom
%%  LocalWords:  ce ar br Euler's te nd exponentials yp ke nt bt ay
%%  LocalWords:  undetceg supereg Cramer's voprandom Wronskian th ne
%%  LocalWords:  varpareg nonhomogeneous sy Bs Ds nT sT nsT Tf lapper
%%  LocalWords:  se lcccl sY sc bs integrable deltaprop exeq integro
%%  LocalWords:  integrations tu VoP sX syseg sysY sysX sZ pseries ty
%%  LocalWords:  tseries LHS serieseg seriesrec Chebyshev FIXME nx mx
%%  LocalWords:  Froebenius singode indicial Bessel's bessel PDE izx
%%  LocalWords:  equidimensional Ogg Vorbis PDEs Eigenfunction heatF
%%  LocalWords:  nicefourier heateq RHS integrand homdibc ic eqn sF
%%  LocalWords:  diribc Neumann nF ds xdx
